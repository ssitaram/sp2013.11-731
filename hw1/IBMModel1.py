...
en_inp = ENINP.readlines()[:10]
ENINP.close()

#source language DE
DEINP = open("data/de.txt", "r")
de_inp = DEINP.readlines()[:10]
DEINP.close()

ten_de_init = init_prob_unit()
ten_de_final = run_EM(5, ten_de_init)
OUT = open("alignments.txt", "w")
OUT.close()
for cond in ten_de.conditions():
    OUT = open("alignments.txt", "a")
    OUT.write(cond+"\t"+ten_de[cond].max()+"\n")
    OUT.close()
